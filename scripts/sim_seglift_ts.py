...
burnin_ts.dump("~/oliviaphd/data/seglift_ts/burnin_seglift_ts.trees")

## FORWARD SIMULATION
# for when uneven seasons " -d g_s=" + str(sum_gen)+ " -d g_w=" + str(win_gen)

## -d simID=
cmd = "slim -d GenomeSize=" + str(int(genomeSize)) + " -d L=" + str(l)+ " -d N=" + str(int(popnSize)) + " -d y=" + str(y) + " -d d=" + str(d) + " -d mut=0.0 -d rr=" + str(recRate) +" ~/oliviaphd/seglift_ts.slim"
print(cmd)
os.system(cmd)

slim_ts = pyslim.load("~/oliviaphd/data/seglift_ts/treeseq_seglift_ts.trees").simplify()
...
mut_freq = np.loadtxt("oliviaphd/data/seglift_ts/sim_data.txt", delimiter = ",", skiprows=(7 + l)) ## import freqs from SLiM
