...
for (g in groups){
  ...
  f_list  <- list.files(path =paste0("~/phd_data/seglift_allele/group_", g, "/"),pattern ="al_freq_")
  
  parameters <- fread(file=paste0("~/phd_data/seglift_allele/group_",g, "/parameters.yml"), sep = ":")
  ...
  for (i in 1:(length(f_list ))){  
    filename = f_list [i]
    #print(filename)
    al_freq  = fread(file = paste0("~/phd_data/seglift_allele/group_",g, "/",filename))
    al_freq [,run := i]
    al_freq [,group:=g]
    al_freq [,d:=dom]
    al_freq [,y:=epi]
