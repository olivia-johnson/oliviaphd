...
for group in range(2, 37):
    with open('/Users/olivia/phd_data/hpc_parameters/single_locus/{1}/group_{0}.txt'.format(group, sim_type), 'r') as f:
    ...
    tmpdir="/Users/olivia/phd_data/Results/single_locus/{1}/group_{0}".format(group, sim_type)
    ...
    tmpdir="~/phd_data/Results/single_locus/{1}/group_{0}".format(group, sim_type)
    results_dir="~/phd_data/Results/single_locus/{1}/group_{0}".format(group, sim_type)
    processes=[]
    
    for sim_run in range(runs):
        sim_run=sim_run
        p=mp.Process(target=single_locus_hpc.simulate_single_locus, args=[tmpdir, results_dir, group, sim_run, recRate, genomeSize, s_pop, w_pop, h_s, h_w, s_s, s_w, rGen, fitness_on, sum_gen, win_gen, freq, sim_type])
        p.start()
        processes.append(p)
